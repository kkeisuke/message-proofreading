...
| CSS | CSS Modules | 学習対象。プリプロセッサもフレームワークも使わない。最新の標準機能を使う（§4） |
...
- スタイルを持つコンポーネントには、同名の CSS Modules ファイルを隣接させる
  - CSS を複数のコンポーネントで共用しない
  - 他のコンポーネントのクラスを外から指定しない。並びや余白は、自分が持つ要素の側で決める
...
- CSS のクラス名は、コンポーネント名を基にした camelCase にする（`messageInputLabel`）
